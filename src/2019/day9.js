...
function getProgramState() {
    ...
    //input_array = [22102, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99];
    // input_array = [109, 19, 204, -34];
    ...
    input_array = computeResult(input_array, 1);

    console.log(JSON.stringify(input_array));
    return input_array[0];
}

function getValueByParameterMode(input_array, index, offset, parameterMode) {
    switch (parameterMode) {
        case 0:
            return input_array[index + 1];
        case 1:
            return index + 1;
        case 2:
            return offset + input_array[index + 1];
        default:
            break;
    }
}

// function getWriteAddress(input_array, index, offset, param_mode) {
//     switch (param_mode) {
//         case 0:
...
function computeResult(input_array, specialInput) {
    ...
    for (let i = 0; i < input_array.length;) {
        ...
        console.log(`firsparammode: ${first_parameter_mode}, secondparammode: ${second_parameter_mode}, thirdparammode: ${third_paramter_mode}, operation: ${operation}`);
        let first_param_pos = getValueByParameterMode(input_array, i, relative_offset, first_parameter_mode);
        first_param_value = input_array[first_param_pos]
        let second_param_pos = getValueByParameterMode(input_array, i + 1, relative_offset, second_parameter_mode);
        second_param_value = input_array[second_param_pos]
        console.log(`firstparamvalue: ${first_param_value}, secondparamvalue: ${second_param_value}`);
        let firstParamRelativePosition = input_array[i + 1] + relative_offset;
        let writeAaddress;
        console.log(`i: ${i}, operation: ${operation}, first_param_value: ${first_param_value}, second_param_value: ${second_param_value}, third_param_mode: ${third_paramter_mode}`);
        switch (operation) {
            case 1:
                writeAaddress = getValueByParameterMode(input_array, i + 3, relative_offset, third_paramter_mode);
                input_array[writeAaddress] = first_param_value + second_param_value;
                i = i + 4;
                break;
            case 2:
                writeAaddress = getValueByParameterMode(input_array, i + 3, relative_offset, third_paramter_mode);
            ...
            case 3:
                // input
                writeAaddress = getValueByParameterMode(input_array, i + 1, relative_offset, first_parameter_mode);
                input_array[writeAaddress] = specialInput;
                i = i + 2;
                break;
            case 4:
                // output
                console.log(`firstparam: ${first_param_value}`);
                i = i + 2;
                break;
            case 5:
            ...
            case 7:
                ...
                if (first_param_value < second_param_value) {
                    input_array[writeAaddress] = 1;
                } else {
                    input_array[writeAaddress] = 0;
                }
                i = i + 4;
                break;
            case 8:
