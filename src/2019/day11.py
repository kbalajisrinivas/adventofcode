    ...
    def paint(self):
        while not self.computer.done:
            ...
            if (self.x, self.y) in self.painted:
                starting_color = self.painted[(self.x, self.y)]
                # print(f"found matching. {self.x}, {self.y}, starting_color: {starting_color}")
                # print(f"===================")
            else:
                starting_color = 0
            ...
            # if self.x == -2 and self.y == -2:
                # print(f"printing for -2 and -2 is {self.painted[(self.x, self.y)]}")
            self.change_direction(self.computer.calculate())
            # print(f"x: {self.x}, y: {self.y}, color: {starting_color}, direction: {self.direction}")
            self.rotate()
...
with open("src/input/day11.txt") as _file:
    for line in _file:
